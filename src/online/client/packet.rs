...
pub const SIZE: usize = HEADER_SIZE + (4+4+1);//PacketHeader + PlayerInput
...
#[derive(Copy,Clone,Debug,Eq,PartialEq)]#[repr(packed)]pub struct PlayerCreateRequest          {pub connection_id: u32_le}                                            impl_FromPacketBytes!(PlayerCreateRequest          : Type);
#[derive(Copy,Clone,Debug,Eq,PartialEq)]#[repr(packed)]pub struct PlayerRemoveRequest          {pub connection_id: u32_le,pub player_network_id: u32_le}              impl_FromPacketBytes!(PlayerRemoveRequest          : Type);
